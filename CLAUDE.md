...
src/
  ...
  shared/settings.js       __BTX.settings  THE owner of the `btxSettings` sync object: SCHEMA/KEYS/defaults (apiKey, provider, enabledTranslations, defaultTranslationId, actOnNonEngOnly, sidebarWidth, scrollToSnippet, citationView, showCitationToggle, citationSourceMark, panelMode, panelCollapsed, scrollSync), one normalizer per setting, normalize/diff (pure), get/patch/replace, subscribe({next,prev,changed,own})
...
- Panel state (mode, citation layout, collapsed, width) has one owner in the
  ...
  enforces it).
- Citation **source marking** (`citationSourceMark`, `'chip'` default | `'strip'`)
  is pure CSS off `#btx-root[data-btx-source-mark]`: `chip` shows the corpus
  acronym tile on each row, `strip` hides it and puts a 3px hue rule on the
  source-type `<details>` (`.btx-cit-cgroup` by-verse, `.btx-cit-vgroup`
  by-source — `cit-panel.groupClass` puts `btx-grp-*` on both). Panel-handled,
  never a re-render (`applyCitSourceMark`); options page is its writer. The
  three chips (count / range / tag) are one 26px tile family — the source hue
  is set once per `btx-tag-*` and `btx-grp-*` (`--btx-src`), never on the count.
