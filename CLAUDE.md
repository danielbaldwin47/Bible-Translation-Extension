...
src/
  ...
  content/
    ...
    panel.js               __BTX.panel  panel DOM, states, mode toggle + citation-layout sub-toggle (setCitationView/setCitationToggleEnabled), scroll-sync, setWidth + drag-resize, setBibleMode
    panel.css
  ...
  citations/
    ...
    cit-panel.js           __BTX.citPanel   two layouts (renderByVerse/renderBySource); anchorVerses dedup; formatVerses/verseLabel range labels; renderByVerse pre-opens the source-type group for single-source verses
  ...
  options/                 options.html/js/css — api.bible key, versions, panel width, citation layout (verse/source), show citation-layout sub-toggle
